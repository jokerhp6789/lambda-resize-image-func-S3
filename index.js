...
function getPathResourceResize(resourcePath) {
  const parts = resourcePath.split("/");
  const option = parts.shift();
  const filename = parts.pop();
  const path = parts.join("/");
  return `${path}/${option}/${filename}`;
}
...
exports.handler = async (event) => {
  ...
  if(parts.length < 2){
    return {
        statusCode: 400,
        body: `${path} invalid. Please contact administrator`,
  ...
  const pathResize = getPathResourceResize(path);
  ...
  let existingResized = await getResource(pathResize);
  if (existingResized) {
